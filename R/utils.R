...
ram_accessToken <- function(accessToken) {
  ...
  if (!is.null(accessToken)) {
    ...
    if(accessToken$expireTime<Sys.time()) stop('Access Token has expired. Please refresh with auth_new_accessToken.', call. = FALSE)
...
ram_checkRefresh = function(refreshToken){
  ...
  if(ls(refreshToken)[[2]]!='refresh_token') stop(paste0('Incorrect object type passed as Refresh Token. Please pass ',
                                                         'the output from auth_init_refreshToken or auth_new_refreshToken.', 
                                                         call. = FALSE))
  ### Check if Refresh Token has expired
  if(refreshToken$refreshExpire<Sys.time()) stop(paste0('The Refresh Token being used has expired. Please reauthenticate ',
